...
const DEBUG=1; //enable to see console.log messages
...
// top button assigned to D31
var pinTopBtn = D31;
// side button assigned to D1
var pinSideBtn = D1;
//var pinSideBtn = BTN1;

//top button clicking commands
var topBtnCommands = {
    "S": "CR",
    "SS": "KP TAB",
    "L": "CAL"
...
// --- pinTopBtn als Eingang mit Pullup ---
...
function moveMouseAction(x, y, b) {
    try {
        ...
        if(DEBUG==2) console.log("Cannot send mouse function, connected as HID device? Reason: " + err.message);
      }
}
...
function holdKeyAction(k) {
    try {
        if(DEBUG == 2) {
            console.log("Holding key: " + k)
        }
        HID.keyDown(k);
    }
    catch (err) {
        digitalPulse(LED1, 1, 300);
        if(DEBUG == 2) {
            console.log("Cannot hold down key. Reason: " + err.message);
        }
    }
}

function releaseKeyAction(k) {
    try {
        if(DEBUG == 1) {
            console.log("Holding key: " + k)
        }
        ...
        digitalPulse(LED1, 1, 300);
        if(DEBUG == 1) {
            console.log("Cannot release key. Reason: " + err.message);
        }
    }
}
...
var sideBtn = new SWBtn(function (k) {
    if(DEBUG==1) console.log("Side button pressed:"+k+", cmd: "+sideBtnCommands[k]);
    executeNextCommand(sideBtnCommands[k]);
}, pinSideBtn);

var topBtn = new SWBtn(function (k) {
    if(DEBUG==1) console.log("Top button pressed:"+k+", cmd: "+topBtnCommands[k]);
    executeNextCommand(topBtnCommands[k]);
}, pinTopBtn);
...
const buttonDebounce = 50; //miliseconds
const InputMode = {
    MOUSE: 0,
    MOVEMENT: 1
};
...
function updateMouseMovementDegree(a) {
    let x = 0, y = 0;
    const sensitivity = 20; // Adjust sensitivity as needed (lower value = higher sensitivity)
    ...
    */
    ...
    //console.log(a.roll + " " + a.pitch);

    // Use tilt level to determine mouse speed and direction
    if ((a.roll - y_calib) > sensitivity) { //neutral at 20
        LED2.set();
        if(currentMode == InputMode.MOUSE){
            //y = speed_roll;
            y=20;
        }
        else if(currentMode == InputMode.MOVEMENT){
            //switch the held down vertical directional key
            try {
                releaseKeyAction(HID.KEY.W);
            } catch (error) {
                if(DEBUG == 1){
                    console.log("Could not release W key: ", error)
                }
            }

            try {
                holdKeyAction(HID.KEY.S);
            } catch (error) {
                if(DEBUG == 1){
                    console.log("Could not hold S key: ", error)
                }
            }
        }
    }
    else if ((a.roll - y_calib) < sensitivity) { //neutral at 0
        LED2.set();
        if(currentMode == InputMode.MOUSE){
            //y = -speed_roll;
            y=-20;
            ...
            try {
                ...
                if(DEBUG == 1){
                    console.log("Could not release S key: ", error)
                }
            }

            try {
                holdKeyAction(HID.KEY.W);
            } catch (error) {
                if(DEBUG == 1){
                    console.log("Could not hold W key: ", error)
                }
            }
        }
    }
    if ((a.pitch - x_calib) > sensitivity) { //neutral at 10
        LED3.set();
        if(currentMode == InputMode.MOUSE){
            //x = speed_pitch;
            x=20;
            ...
            //switch the held down horizontal directional key
            try {
                releaseKeyAction(HID.KEY.A);
            } catch (error) {
                if(DEBUG == 1){
                    console.log("Could not release A key: ", error)
                }
            }

            try {
                holdKeyAction(HID.KEY.D);
            } catch (error) {
                if(DEBUG == 1){
                    console.log("Could not hold D key: ", error)
                }
            }
        }
    }
    else if ((a.pitch - x_calib) < -sensitivity) { //neutral at -10
        LED3.set();
        if(currentMode == InputMode.MOUSE){
            //x = -speed_pitch;
            x=-20;
            ...
            try {
                ...
                if(DEBUG == 1){
                    console.log("Could not release D key: ", error)
                }
            }

            try {
                holdKeyAction(HID.KEY.A);
            } catch (error) {
                if(DEBUG == 1){
                    console.log("Could not hold A key: ", error)
                }
            }
        }
    }
    else{
        //neutral position -> release held down keys
        try {
            releaseKeyAction(HID.KEY.W);
        } catch (error) {
            if(DEBUG == 1){
...
function changeMode() {
    if(currentMode == InputMode.MOUSE){
        currentMode = InputMode.MOVEMENT;
        if(DEBUG == 1){
            console.log("Switched to movement");
        }
    }
    else if(currentMode == InputMode.MOVEMENT){
        currentMode = InputMode.MOUSE;
        if(DEBUG == 1){
            console.log("Switched to mouse");
        }
    }
}

function calibrateMouse() {
    x_calib = AHRS.getOrientationDegree().roll.toFixed(2);
    y_calib = AHRS.getOrientationDegree().pitch.toFixed(2);
}
